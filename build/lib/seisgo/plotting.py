...
def plot_stations(lon,lat,region=None,markersize="c0.2c",title="station map",style="fancy",figname=None,
                  format='png',projection="M5i", frame="af",save=False):
    raise ValueError('plot_stations has been renamed to gmt_scatters(). Usage is the same.')
def gmt_scatters(lon,lat,region=None,markersize="c0.2c",title="map",style="fancy",figname=None,
                  format='png',color_water="cyan",color_land="gray",color_symbol="red",
                  projection="M5i", frame="af",save=False):
    """
    ...
    for i in range(nsta):
        if i==0:
            fig.coast(region=region, resolution="f",projection=projection, rivers='rivers',
                      water=color_water,frame=frame,land=color_land,
